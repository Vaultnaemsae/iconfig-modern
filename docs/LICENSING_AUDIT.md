...
| legacy icons/device images | files from official GPL source snapshot; names and product imagery identify iConnectivity hardware | source-license grant exists, but trademark/endorsement rights are separate | source preservation is supportable; using legacy branding as the new public binary identity is not cleared by GPL alone | retain as historical source; obtain/review a fork-specific modern icon before release |
